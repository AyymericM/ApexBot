...
module.exports = {
	...
	usage: '[command name]',
  	execute(client, msg, args) {
		...
		const name = args[1].toLowerCase(); // get the command's name and convert it to lowercase.
		console.log(args)
		const command =
			commands.get(name) ||
		...
		if (!command) {
			return msg.reply("that's not a valid command!"); // if there is no command found, report that the command doesn't exist.
		}
